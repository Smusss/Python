...
async def mes_yes(message:types.Message):
    await message.answer("Let's get ready to rumble!\n"
                         ...
                         "Input '/set_bank' and count of sweets ")
    game = True
...
@dp.message_handler(commands=['set_bank'])
async def mes_set(message:types.Message):
    count = message.text.split()[1]
    sweets_bank = int(count)
    await message.answer(f'Sweets bank = {sweets_bank}.\n'
                          "Input '/set_max' and count of max count of sweets that can be taken by turn.")
    return sweets_bank
...
async def mes_go(message:types.Message, game, sweets_bank, max_take, turn):
    if game is False:
        ...
        if turn == 1:
            await bot.send_message(message.from_user.id, f'Your turn. There are {sweets_bank} on the table. \n*How many sweets will you take?*')
        else:
...
@dp.message_handler()
async def mes_all(message:types.Message, check, sweets_bank, max_take, turn):
    while sweets_bank > 0:
        if turn == 1:
        await bot.send_message(message.from_user.id, f'Your turn. There are {sweets_bank} on the table. \n*How many sweets will you take?*')
        else:
